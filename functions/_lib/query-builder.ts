...
export function buildGetActiveCodeCountQuery(batchId: string, env: CloudflareEnv) {
  return env.SOULTEST_DB.prepare("SELECT COUNT(*) AS value FROM codes WHERE batch_id = ?1 AND status = 'active'")
    .bind(batchId)
    .first<{ value: number }>()
}
...
export function buildGetActiveCodesCountQuery(env: CloudflareEnv) {
  return env.SOULTEST_DB.prepare("SELECT COUNT(*) AS value FROM codes WHERE status = 'active'").first<{ value: number }>()
}

export function buildGetTotalSubmissionsQuery(env: CloudflareEnv) {
  return env.SOULTEST_DB.prepare("SELECT COUNT(*) AS value FROM submissions").first<{ value: number }>()
}

export function buildGetSubmissions24hQuery(env: CloudflareEnv) {
  return env.SOULTEST_DB.prepare("SELECT COUNT(*) AS value FROM submissions WHERE datetime(created_at) >= datetime('now', '-1 day')").first<{ value: number }>()
}

export function buildGetSubmissions7dQuery(env: CloudflareEnv) {
  return env.SOULTEST_DB.prepare("SELECT COUNT(*) AS value FROM submissions WHERE datetime(created_at) >= datetime('now', '-7 day')").first<{ value: number }>()
}

export function buildGetSubmissions30dQuery(env: CloudflareEnv) {
  return env.SOULTEST_DB.prepare("SELECT COUNT(*) AS value FROM submissions WHERE datetime(created_at) >= datetime('now', '-30 day')").first<{ value: number }>()
}

export function buildGetAvgDurationQuery(env: CloudflareEnv) {
  return env.SOULTEST_DB.prepare("SELECT ROUND(AVG(duration_sec)) AS avgDurationSec FROM submissions WHERE duration_sec IS NOT NULL").first<{ avgDurationSec: number | null }>()
}

export function buildGetShareCountQuery(env: CloudflareEnv) {
  return env.SOULTEST_DB.prepare("SELECT COUNT(*) AS shareCount FROM submissions WHERE shared = 1").first<{ shareCount: number }>()
}
...
export function buildGetDailySubmissionsQuery(env: CloudflareEnv) {
  return env.SOULTEST_DB.prepare(
    `
      SELECT
...
export function buildGetCodeCountQuery(batchId: string, env: CloudflareEnv) {
  return env.SOULTEST_DB.prepare(
    `
...
export function buildDeleteCodesByBatchQuery(batchId: string, env: CloudflareEnv) {
  return env.SOULTEST_DB.prepare(`DELETE FROM codes WHERE batch_id = ?1`).bind(batchId).run()
}

export function buildDeleteCodeBatchQuery(batchId: string, env: CloudflareEnv) {
  return env.SOULTEST_DB.prepare(`DELETE FROM code_batches WHERE id = ?1`).bind(batchId).run()
}
...
export function buildUpdateCodeBatchStatusQuery(batchId: string, status: string, env: CloudflareEnv) {
  return env.SOULTEST_DB.prepare(
    `
      UPDATE code_batches
      SET status = ?2
